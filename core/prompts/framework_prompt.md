...
PROVIDED SCAFFOLDING — DO NOT CREATE, OVERWRITE, OR EDIT: `conftest.py` (project root), `pages/base_page.py`, and `utils.py` (project root) are already in place (the harness copies them in). `conftest.py` provides the Playwright `browser`/`context`/`page` fixtures (headless unless `--headed`), `base_url`, `test_data`, `global_data`, `captured_values`, `scenario_context`, `tabs`, and the pytest-bdd step hooks that write `report/step_trace.json` and `report/captured_values.json`. `utils.py` provides shared test utilities — import directly in any step def: `from utils import unique_suffix`. Do NOT write or modify `conftest.py`, `pages/base_page.py`, or `utils.py`, and do NOT touch `pytest_plugins` — the harness manages it. Your job is ONLY the project-specific files: `mcp-selectors/locators.json`, `pages/page_<slug>.py` (inherit `BasePage`), `step_defs/<slug>_steps.py`, and `test/test_<slug>.py`. Write these in that order (locators → page objects → step defs → tests) so the core deliverables land first.

...
`conftest.py`, `pages/base_page.py`, and `utils.py` are PROVIDED SCAFFOLDING (copied in by the harness) — never create, overwrite, edit, or delete them, and never touch `pytest_plugins`. They already provide the fixtures, pytest-bdd step hooks (`report/step_trace.json`, `report/captured_values.json`), and shared utilities (`from utils import unique_suffix` for unique names per test run — store the result in `scenario_context` to share it across steps). Generate only the project-specific page objects, step defs, and tests.
═══════════════════════════════════════════════════════════════════

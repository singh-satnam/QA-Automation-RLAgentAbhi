...
class ProjectCreationPage(BasePage):
    def click_add_new(self):
        btn = self.page.locator(self.loc["create_project"]["add_new_button"])
    ...
    def select_user(self, user_label):
        # Angular custom checkbox: actual <input> is off-screen; trigger via JS click
        checkbox = self.page.get_by_role("checkbox", name=user_label, exact=True)
        checkbox.wait_for(state="attached", timeout=15000)
        if not checkbox.is_checked():
            checkbox.evaluate("el => el.click()")

    def select_catalog_type(self, label_contains):
        # Match checkbox by partial accessible name; exact=False does substring match
        checkbox = self.page.get_by_role("checkbox", name=label_contains, exact=False)
        checkbox.first.wait_for(state="attached", timeout=15000)
